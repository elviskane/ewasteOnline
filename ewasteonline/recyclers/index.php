<!DOCTYPE html>
<html>
    <head>
    <title>About</title>
   <?php include'modules/header.php';?>
</head>
    <body>
       
        <?php include 'modules/navigate.php';?>
        <div id="client_mainContent">
        <?php include'modules/sidebarnav.php';?>
        <div class="submain_content">
              <?php include'discart.php'?>
            <div class="container-fluid">
               
               <h2 style="margin-left:30px;">About Us</h2>
                <div class="col-md-12">
                   <div class="col-md-12"><img class="ewaste_img" src="../images/main.jpg"/></div>
                    <div class="col-md-12 test"><h4>We as an E-waste company Hope to contribute to environmental efforts of other scientists and none-scientist alike with aim of conserving the environment we live in . Our Role is to collect waste materials produced to make our environment more habitable and livable.</h4></div>
                </div>
                <div class="display_items col-md-12 text-center">
                    <div class="col-md-3">
                        <h3>Our Services</h3>
                        <h4>Offer PickUp Services</h4>
                        <h4>Data Destruction</h4>
                        <h4>E-waste Sorting</h4>
                        <h4>E-waste Standards Compliance</h4>
                        <h4>E-waste Disposal</h4>
                    </div>
                 <div class="col-md-3">
                     <h3>Our Prices</h3>
                     <h4>Our prices can be found at the device section.this contains offers we make irreguardless of device state.Same applies to bulk e-waste</h4>
                    </div>
                 <div class="col-md-3">
                     <h3>Vision</h3>
                      <h4>Reduces Toxic Waste</h4>
                      <h4>Protect Human Health</h4>
                      <h4>Reduce Strain On Natural resource By Recycling</h4>
                      <h4>Maintain life support conditions</h4>
                     
                    </div>
                 <div class="col-md-3">
                     <h3>Benefits</h3>
                    <h4>Reduces Toxic Waste</h4>
                      <h4>Protect Human Health</h4>
                      <h4>Offer Buyback Value</h4>
                      <h4>Maintain life support conditions</h4>
                    </div>
                </div>
              
            </div>
        </div>
            <?php include'modules/footer.php';?>
    </div>   
     
    </body>


</html>